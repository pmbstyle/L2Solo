...
function loadedItems() {
    return [
        ...
        'data/Items/Others/c4_devils_isle.json'
...
const spawnRows = tuples('sql/spawnlist.sql').filter((row) => row[1] === 'Elmore_Northeast_Coast');
if (spawnRows.length !== 283) throw new Error(`Expected 283 northeast coast spawns, found ${spawnRows.length}`);
if (spawnRows.some((row) => npcRowsById.get(Number(row[3]))?.[11] !== 'L2Monster')) {
    throw new Error('Elmore northeast coast source label contains a non-monster row');
}
if (spawnRows.some((row) => Number(row[2]) !== 1 || Number(row[10]) !== 120 || Number(row[12]) !== 0)) {
    throw new Error('Unexpected northeast coast spawn count, respawn, or period semantics');
}
const mobIds = [...new Set(spawnRows.map((row) => Number(row[3])))].sort((a, b) => a - b);
const expectedMobIds = [1124, 1125, 1126, 1127, 1128, 1129, 1130];
if (JSON.stringify(mobIds) !== JSON.stringify(expectedMobIds)) {
    throw new Error(`Expected monster ids ${expectedMobIds.join(', ')}, found ${mobIds.join(', ')}`);
}
...
if (skillRows.length !== 13) throw new Error(`Expected 13 NPC skill rows, found ${skillRows.length}`);
const raceBySkill = new Map([[4291, 'construct'], [4292, 'beast'], [4295, 'humanoid'], [4301, 'insect']]);
...
const spawnDefinitions = mobIds.map((npcId) => {
    ...
    return {
        selfId: npcId, name: npcNameById.get(npcId),
        coords: rows.map((row) => ({ locX: row[4], locY: row[5], locZ: row[6], head: row[9] })),
        total: 1, respawn: 120, bias: 0
    };
});
const spawns = [{ selfId: 'c4-elmore-northeast-coast', bounds: [], spawns: spawnDefinitions }];

const dropRows = tuples('sql/droplist.sql').filter((row) => mobIdSet.has(Number(row[0])));
if (dropRows.length !== 83) throw new Error(`Expected 83 drops, found ${dropRows.length}`);
...
const rewards = mobIds.map((mobId) => {
    ...
    const spoils = rows.filter((row) => Number(row[4]) === -1).map((row) => ({
        items: [{ selfId: Number(row[1]), name: sourceItemName(Number(row[1])), min: Number(row[2]),
...
const missingItems = [...requiredItemIds].filter((id) => !existingItemsById.has(id)).sort((a, b) => a - b).map((id) => {
    ...
    return {
        selfId: id,
        template: { kind: itemKind(source), name: source.name, class1: 4, class2: 0,
...
const expectedMissingItemIds = [5273, 6035];
...
writeJson('data/Npcs/c4_elmore_northeast_coast.json', npcs);
writeJson('data/Npcs/Spawns/c4_elmore_northeast_coast.json', spawns);
writeJson('data/Npcs/Rewards/c4_elmore_northeast_coast.json', rewards);
writeJson('data/Items/Others/c4_elmore_northeast_coast.json', missingItems);
writeJson('data/Npcs/Skills/c4_elmore_northeast_coast.json', skillRows);
