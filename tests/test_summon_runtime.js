...
(async () => {
    ...
    session.summon = null;
    session.pet = null;
    const striderItem = item(990059, 4422, 1);
    session.actor.backpack.items.push(striderItem);
    const oldPersistenceMode = session.persistenceMode;
    session.persistenceMode = 'ephemeral';
    World.npc = { spawns: [], grid: {}, nextId: 990060 };
    session.actor.backpack.spawnPetFromItem(session, { itemObjectId: 990059, npcId: 12526 });
    const strider = session.actor.pet;
    assert(strider, 'persistent strider spawned');
    ...
    assert.strictEqual(World.npc.spawns.includes(strider), false, 'dismount returns the strider to its control item');
    assert.strictEqual(session.actor.pet, null);
    await strider.teardownTail;
    assert.strictEqual(striderItem.petInUse, false);
    session.persistenceMode = oldPersistenceMode;
