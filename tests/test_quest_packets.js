...
assert.strictEqual(sent[0].readUInt16LE(1), 1, 'starting or advancing a quest must refresh the journal');
...
QuestService.syncActiveQuests(session);
assert.strictEqual(sent[1].readUInt16LE(1), 0, 'completing a quest must remove it from the client journal');

const received = [];
const rewardSession = { dataSendToMe: (packet) => received.push(packet) };
QuestService.transmitItemReceived(rewardSession, 57, 22500);
assert.strictEqual(received[0][0], 0x64, 'quest rewards must use C4 SystemMessage');
assert.strictEqual(received[0].readInt32LE(1), 52, 'Adena rewards must show the earned-Adena message');
...
assert.strictEqual(received[1].readInt32LE(1), 54, 'single-item rewards must show the earned-item message');
...
assert.strictEqual(soundPacket[0], 0x98, 'quest state changes must use the C4 PlaySound packet');
assert.strictEqual(soundPacket.readInt32LE(1), 0, 'quest sounds must be immediate client sounds');
assert.strictEqual(soundPacket.subarray(5, 5 + 'ItemSound.quest_finish'.length * 2).toString('ucs2'), 'ItemSound.quest_finish');
