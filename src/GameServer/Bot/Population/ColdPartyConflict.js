...
async function apply({ event, life, owner, memory, parties, personaFor, participantAllowed,
    contestContextAllowed, onState, now, waitMs, cooldownMs, pvpEnabled = () => false, rng = seeded(event.key) }) {
    ...
    const { roles, deescalated } = select(sides, memory, personaFor, rng, timestamp);
    const pvp = !deescalated && event.pvpIntent === true && pvpEnabled()
        ? require('./ColdPvpResolver').resolve({ sides, roles, timestamp, rng, personaFor }) : null;
    ...
    const wait = { start: timestamp, until: pvp?.started ? pvp.until : timestamp + waitMs,
    ...
    const next = sides.flatMap((side, index) => side.members.map(state => {
        ...
        return { ...result, stats: { ...result.stats, coldCompetition: { ...state.stats?.coldCompetition, ...episode,
    ...
    if (pvp?.started) for (const incident of pvp.incidents) {
        ...
        for (const type of incident.killed ? ['attacked', 'killed'] : ['attacked']) {
            events.push({ key: `${event.key}:${sourceId}:${targetId}:${type}`, sourceId, targetId,
                kind: 'character', type, at: timestamp });
        }
    }
    ...
    try {
        ...
        return { ok: true, deescalated, outcome, pvp: !!pvp?.started,
            ...
            ...(pvp?.started ? { combat: { durationMs: pvp.durationMs, actions: pvp.actions, fighters: pvp.fighters } } : {}),
