...
function startLink() {
  return '<a action="bypass -h quest 4 start">I will gather the six gifts.</a>';
}

function giftCount(state) {
  return [...GIFTS.values()].reduce(
    (count, itemId) =>
      count +
      (state.session.actor.backpack
        .fetchItemFromSelfId(itemId)
        ?.fetchAmount() || 0),
    0,
  );
}
...
module.exports = {
  ...
  async onEvent(state, event) {
    ...
    if (Number(actor.fetchRace()) !== 3 || Number(actor.fetchLevel()) < 2)
      return null;
    ...
    return page(
      "Nakusin",
      "Visit the six elders and collect their gifts for Lord Pa’agrio.",
    );
  },
  ...
  async onTalk(state, npc) {
    ...
    if (!state.isStarted()) {
      ...
      if (Number(actor.fetchRace()) !== 3)
        return page("Nakusin", "This duty is for Orcs only.");
      return Number(actor.fetchLevel()) < 2
        ? page("Nakusin", "Come back after reaching level 2.")
        : page(
            "Nakusin",
            "Will you bring the six gifts for Lord Pa’agrio?",
            startLink(),
          );
    }
    ...
    if (npcId === NAKUSIN) {
      if (state.getInt("cond") === 1)
        return page("Nakusin", "Bring me all six gifts.");
      ...
      return page(
        "Nakusin",
        "You have served Lord Pa’agrio well. Please accept this Club.",
      );
    }
    ...
    return page(
      "Quest",
      "You received an elder’s gift. Return to Nakusin after collecting all six.",
    );
  },
};
