...
function hasNote(state) {
  return (
    (state.session.actor.backpack
      .fetchItemFromSelfId(ROSELYN_NOTE)
      ?.fetchAmount() || 0) > 0
  );
}
...
module.exports = {
  ...
  async onEvent(state, event) {
    ...
    if (event === "start" && !state.isStarted() && !state.isCompleted()) {
      if (Number(actor.fetchRace()) !== 2 || Number(actor.fetchLevel()) < 3)
        return null;
    ...
    if (
      ...
      if (!hasNote(state))
        return page("Harne", "You do not have Roselyn’s note.");
  ...
  async onTalk(state, npc) {
    ...
    if (!state.isStarted()) {
      ...
      if (Number(actor.fetchRace()) !== 2 || Number(actor.fetchLevel()) < 3)
        return page(
          "Jasmine",
          "This adventure is for Dark Elves of level 3 or higher.",
        );
      return page(
        "Jasmine",
        "Would you like to begin your adventure?",
        '<a action="bypass -h quest 8 start">Begin.</a>',
      );
    }
    ...
    if (npcId === JASMINE) {
      if (cond < 3)
        return page("Jasmine", "Please speak with Roselyn and Harne.");
      return page(
        "Jasmine",
        "Welcome back.",
        '<a action="bypass -h quest 8 reward">Receive the traveler’s mark.</a>',
      );
    }
    if (npcId === ROSELYN) {
      return cond === 1
        ? page(
            ...
            '<a action="bypass -h quest 8 note">Take the note.</a>',
          )
    ...
    if (npcId === HARNE) {
      return cond === 2
        ? page(
            "Harne",
            "Do you have Roselyn’s note?",
            '<a action="bypass -h quest 8 deliver">Deliver the note.</a>',
          )
