...
module.exports = {
    id: 17, name: 'Light and Darkness', npcs: [HIERARCH, ...ALTARS], startNpcs: [HIERARCH],
    eventNpc(event) {
    ...
    async onEvent(state, event) {
        ...
        if (event === 'start' && !state.isStarted() && !state.isCompleted()) {
            ...
            await service().giveItem(state.session, BLOOD, 4); state.playSound(SOUND_ACCEPT);
    ...
    async onTalk(state, npc) {
        ...
        if (!state.isStarted()) return npcId === HIERARCH ? (Number(actor.fetchLevel()) < 61 ? page('Hierarch', 'Will you bring light to the Saint Altars?', '<a action="bypass -h quest 17 start">Accept.</a>') : page('Hierarch', 'This task is for adventurers below level 61.')) : page('Quest', 'You are not on a quest that involves this NPC.');
        const cond = state.getInt('cond');
        if (npcId === HIERARCH) {
            if (cond !== 5) return page('Hierarch', 'Continue to the next Saint Altar.');
            service().rewardExpSp(state.session, 105527, 0); state.playSound(SOUND_FINISH); await state.exit(false);
            return page('Hierarch', 'The Saint Altars shine again.');
        }
        const step = ALTARS.indexOf(npcId) + 1;
        if (cond === step) return page(`Saint Altar ${step}`, 'Offer the Blood of a Saint.', `<a action="bypass -h quest 17 altar${step}">Consecrate the altar.</a>`);
        return page(`Saint Altar ${step}`, cond > step ? 'This altar is already consecrated.' : 'Consecrate the previous altar first.');
    }
};
