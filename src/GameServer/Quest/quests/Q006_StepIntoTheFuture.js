...
function hasLetter(state) { return (state.session.actor.backpack.fetchItemFromSelfId(BAULRO_LETTER)?.fetchAmount() || 0) > 0; }
...
module.exports = {
    ...
    async onEvent(state, event) {
        ...
        if (event === 'start' && !state.isStarted() && !state.isCompleted()) {
            if (Number(actor.fetchRace()) !== 0 || Number(actor.fetchLevel()) < 3) return null;
        ...
        if (event === 'deliver' && state.isStarted() && state.getInt('cond') === 2) {
            if (!hasLetter(state)) return page('Sir Collin', 'You do not have Baulro’s letter.');
    ...
    async onTalk(state, npc) {
        ...
        if (!state.isStarted()) {
            if (npcId !== ROXXY) return page('Quest', 'You are not on a quest that involves this NPC.');
            if (Number(actor.fetchRace()) !== 0 || Number(actor.fetchLevel()) < 3) return page('Roxxy', 'This journey is for Humans of level 3 or higher.');
            return page('Roxxy', 'Would you like to begin your journey?', '<a action="bypass -h quest 6 start">Begin.</a>');
        }

        const cond = state.getInt('cond');
        if (npcId === ROXXY) {
            if (cond < 3) return page('Roxxy', 'Please speak with Baulro and Sir Collin.');
            return page('Roxxy', 'Welcome back.', '<a action="bypass -h quest 6 reward">Receive the traveler’s mark.</a>');
        }
        if (npcId === BAULRO) {
            return cond === 1
                ? page('Baulro', 'I have a letter for Sir Collin.', '<a action="bypass -h quest 6 letter">Take the letter.</a>')
                : page('Baulro', 'Please continue to Sir Collin.');
        }
        if (npcId === SIR_COLLIN) {
            return cond === 2
                ? page('Sir Collin', 'Do you have Baulro’s letter?', '<a action="bypass -h quest 6 deliver">Deliver the letter.</a>')
