...
module.exports = {
  ...
  async onEvent(state, event) {
    ...
    if (event === "start" && !state.isStarted() && !state.isCompleted()) {
      if (Number(actor.fetchRace()) !== 3 || Number(actor.fetchLevel()) < 3)
        return null;
      ...
      return page(
        "Petukai",
        "Seek Tanapi’s counsel before travelling to the City of Humans.",
      );
    }
    if (
      event === "council" &&
  ...
  async onTalk(state, npc) {
    ...
    if (!state.isStarted()) {
      ...
      if (Number(actor.fetchRace()) !== 3 || Number(actor.fetchLevel()) < 3)
        return page(
          "Petukai",
          "This journey is for Orcs of level 3 or higher.",
        );
      return page(
        "Petukai",
        "Would you like to begin your journey?",
        '<a action="bypass -h quest 9 start">Begin.</a>',
      );
    }
    ...
    if (npcId === TANAPI) {
      return cond === 1
        ? page(
            "Tanapi",
            "I will advise you.",
            '<a action="bypass -h quest 9 council">Receive Tanapi’s counsel.</a>',
          )
    ...
    if (npcId === TAMIL) {
      return cond === 2
        ? page(
            ...
            '<a action="bypass -h quest 9 reward">Receive the traveler’s mark.</a>',
          )
