...
module.exports = {
  ...
  async onEvent(state, event) {
    ...
    if (event === "start" && !state.isStarted() && !state.isCompleted()) {
      ...
      await service().giveItem(state.session, CRYSTAL, 5);
    ...
    if (event === "reward" && state.isStarted() && state.getInt("cond") === 6) {
      service().rewardExpSp(state.session, 221958, 0);
  ...
  async onTalk(state, npc) {
    ...
    if (!state.isStarted())
      return npcId === HIERARCH
        ? Number(actor.fetchLevel()) < 62
          ? page(
              "Hierarch",
              "Will you confront the coming darkness?",
              '<a action="bypass -h quest 16 start">Accept.</a>',
            )
          : page("Hierarch", "This task is for adventurers below level 62.")
    ...
    if (npcId === HIERARCH)
      return cond === 6
        ? page(
            "Hierarch",
            "All five altars are purified.",
            '<a action="bypass -h quest 16 reward">Report success.</a>',
          )
        : page("Hierarch", "Continue to the next Evil Altar.");
    const step = ALTARS.indexOf(npcId) + 1;
    if (cond === step)
      return page(
        `Evil Altar ${step}`,
        "Use a Crystal of Seal.",
        `<a action="bypass -h quest 16 altar${step}">Purify the altar.</a>`,
      );
    return page(
      `Evil Altar ${step}`,
      cond > step
        ? "This altar is already purified."
        : "Purify the previous altar first.",
    );
  },
};
