...
function hasRecommendation(state) { return (state.session.actor.backpack.fetchItemFromSelfId(ARIEL_RECOMMENDATION)?.fetchAmount() || 0) > 0; }
...
module.exports = {
    ...
    async onEvent(state, event) {
        ...
        if (event === 'start' && !state.isStarted() && !state.isCompleted()) {
            if (Number(actor.fetchRace()) !== 1 || Number(actor.fetchLevel()) < 3) return null;
        ...
        if (event === 'recommendation' && state.isStarted() && state.getInt('cond') === 1) {
        ...
        if (event === 'deliver' && state.isStarted() && state.getInt('cond') === 2) {
            if (!hasRecommendation(state)) return page('Asterios', 'You do not have Ariel’s recommendation.');
    ...
    async onTalk(state, npc) {
        ...
        if (!state.isStarted()) {
            if (npcId !== MIRABEL) return page('Quest', 'You are not on a quest that involves this NPC.');
            if (Number(actor.fetchRace()) !== 1) return page('Mirabel', 'This journey is for Elves only.');
            if (Number(actor.fetchLevel()) < 3) return page('Mirabel', 'Come back after reaching level 3.');
            return page('Mirabel', 'Would you like to begin your journey?', '<a action="bypass -h quest 7 start">Begin.</a>');
        }

        const cond = state.getInt('cond');
        if (npcId === MIRABEL) {
            if (cond < 3) return page('Mirabel', 'Please speak with Ariel and Asterios.');
            return page('Mirabel', 'Welcome back.', '<a action="bypass -h quest 7 reward">Receive the traveler’s mark.</a>');
        }
        if (npcId === ARIEL) {
            return cond === 1
                ? page('Ariel', 'I can prepare a recommendation for Asterios.', '<a action="bypass -h quest 7 recommendation">Take the recommendation.</a>')
                : page('Ariel', 'Please continue to Asterios.');
        }
        if (npcId === ASTERIOS) {
            return cond === 2
                ? page('Asterios', 'Do you have Ariel’s recommendation?', '<a action="bypass -h quest 7 deliver">Deliver the recommendation.</a>')
