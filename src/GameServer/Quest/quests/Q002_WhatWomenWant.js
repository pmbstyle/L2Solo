...
function startLink() {
  return '<a action="bypass -h quest 2 start">I will help you.</a>';
}
...
module.exports = {
  ...
  startNpcs: [ARUJIEN],
  eventNpc: (event) =>
    ["start", "poetry", "reward"].includes(event) ? ARUJIEN : null,
  ...
  async onEvent(state, event) {
    ...
    if (event === "start" && !state.isStarted() && !state.isCompleted()) {
      ...
      if (
        Number(actor.fetchLevel()) < 2 ||
        ![0, 1].includes(Number(actor.fetchRace()))
      )
        return null;
  ...
  async onTalk(state, npc) {
    ...
    if (!state.isStarted()) {
      ...
      if (![0, 1].includes(Number(actor.fetchRace())))
        return page("Arujien", "I only need the help of a Human or an Elf.");
      return Number(actor.fetchLevel()) < 2
        ? page("Arujien", "Come back after reaching level 2.")
        : page("Arujien", "I need help with a delicate matter.", startLink());
    }
    ...
    if (npcId === ARUJIEN) {
      ...
      if (cond === 3)
        return page(
          "Arujien",
          "What did Herbiel say?",
          '<a action="bypass -h quest 2 poetry">Give him the poetry book.</a><br><a action="bypass -h quest 2 reward">Give him the reply and finish.</a>',
        );
      if (cond === 4)
        return page("Arujien", "Please deliver the poetry book to Greenis.");
      if (cond === 5) {
        ...
        return page(
          "Arujien",
          "Thank you. Please accept these beginner potions.",
        );
      }
    }
    return null;
