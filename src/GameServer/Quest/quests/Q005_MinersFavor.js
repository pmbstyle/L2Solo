...
function startLink() { return '<a action="bypass -h quest 5 start">I will collect the supplies.</a>'; }
function has(state, itemId) { return (state.session.actor.backpack.fetchItemFromSelfId(itemId)?.fetchAmount() || 0) > 0; }
function hasAll(state, items = REQUIRED) { return items.every((itemId) => has(state, itemId)); }
...
module.exports = {
    ...
    startNpcs: [BOLTER],
    eventNpc: (event) => event === 'start' ? BOLTER : (event === 'pick' ? BRUNON : null),
    ...
    async onEvent(state, event) {
        if (event === 'start' && !state.isStarted() && !state.isCompleted()) {
            ...
            return page('Bolter', 'Bring me mining boots, a miner’s pick, boomboom powder, and Redstone beer.');
        }
        if (event === 'pick' && state.isStarted() && state.getInt('cond') === 1 && has(state, SOCKS) && !has(state, PICK)) {
            await service().takeItem(state.session, SOCKS);
            await grantSupply(state, PICK);
            return page('Brunon', 'These socks are unbearable. Take this miner’s pick.');
        }
        return null;
    },
    ...
    async onTalk(state, npc) {
        ...
        if (!state.isStarted()) {
            if (npcId !== BOLTER) return page('Quest', 'You are not on a quest that involves this NPC.');
        ...
        if (npcId === BOLTER) {
            if (cond === 1) return page('Bolter', 'Please bring the four mining supplies.');
            ...
            return page('Bolter', 'Thank you. Please accept this Necklace of Knowledge.');
        }
        if (cond !== 1) return page('Quest', 'Return to Bolter.');

        if (npcId === SHARI) {
            if (has(state, POWDER)) return page('Shari', 'You already have the boomboom powder.');
            await grantSupply(state, POWDER);
            return page('Shari', 'Here is the boomboom powder.');
        }
        if (npcId === GARITA) {
            if (has(state, BOOTS)) return page('Garita', 'You already have the mining boots.');
            await grantSupply(state, BOOTS);
            return page('Garita', 'Here are the mining boots.');
        }
        if (npcId === REED) {
            if (has(state, BEER)) return page('Reed', 'You already have the Redstone beer.');
        ...
        if (npcId === BRUNON) {
            return has(state, PICK)
                ...
                : page('Brunon', 'I will trade a miner’s pick for Bolter’s smelly socks.', '<a action="bypass -h quest 5 pick">Give him the socks.</a>');
        }
        return null;
    }
};
