...
module.exports = {
    ...
    async onEvent(state, event) {
        if (event !== 'start' || state.isCompleted() || state.isStarted()) return null;
    ...
    async onTalk(state, npc) {
        ...
        if (!state.isStarted()) {
            return npcId === DARIN
                ? (level < 2 ? page('Darin', 'Come back after reaching level 2.') : page('Darin', 'Could you help me deliver a letter?', startLink()))
        ...
        if (npcId === DARIN) {
            if (cond === 1) return page('Darin', 'Please deliver my letter to Roxxy.');
            if (cond === 2) {
