...
async function collect(state, itemId) {
  ...
  const complete = [EYE, STONE, BLOOD].every(
    (id) =>
      state.session.actor.backpack.fetchItemFromSelfId(id)?.fetchAmount() >= 1,
  );
...
module.exports = {
  ...
  async onEvent(state, event) {
    ...
    if (Number(actor.fetchRace()) !== 2 || Number(actor.fetchLevel()) < 16)
      return null;
    ...
    return page(
      "Talloth",
      "Bring me an Onyx Beast Eye, a Taint Stone, and Succubus Blood.",
    );
  },
  ...
  async onTalk(state, npc) {
    ...
    if (!state.isStarted()) {
      if (Number(actor.fetchRace()) !== 2)
        return page("Talloth", "This task is for Dark Elves only.");
      return Number(actor.fetchLevel()) < 16
        ? page("Talloth", "Come back after reaching level 16.")
        : page(
            "Talloth",
            "Will the seal be broken?",
            '<a action="bypass -h quest 3 start">Accept the task.</a>',
          );
    }
    if (state.getInt("cond") === 1)
      return page(
        "Talloth",
        "Keep hunting until you have all three ritual ingredients.",
      );
    const Quest = service();
    const backpack = state.session.actor.backpack;
    const hasAllItems = [EYE, STONE, BLOOD].every(
      (id) => (backpack.fetchItemFromSelfId(id)?.fetchAmount() || 0) >= 1,
    );
    if (!hasAllItems)
      return page(
        "Talloth",
        "You must bring all three ritual ingredients before I can reward you.",
      );
    ...
    return page(
      "Talloth",
      "The seal has been broken. You receive 4,900 Adena and 5,000 XP.",
    );
  },
